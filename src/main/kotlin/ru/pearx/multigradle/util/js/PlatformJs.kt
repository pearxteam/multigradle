    ...
    override fun Project.configureAfter(module: Project, root: Project, extension: MultiGradleJsExtension)
    {
        ...
        tasks {
            ...
            create<NodeTask>("runMocha") {
                ...
                setEnvironment(mapOf("NODE_PATH" to "$buildDir/kotlinjs", "JUNIT_REPORT_PATH" to "$buildDir/test-results/test/mocha.xml"))
                setArgs(listOf(getByName<Kotlin2JsCompile>("compileTestKotlin2Js").destinationDir, "--reporter", "mocha-jenkins-reporter"))
            }
